...
성능에 치명적인 영향을 미치는 코드는 많지 않으며, 그래서 성급한 최적화는 해서는 안된다. "아니 장난해? 그렇다면 성능 얘기는 뭐하러 꺼낸거야?"라고 말할 수도 있을텐데, 일반적으로 `Stream.forEach()`를 사용하면 전통적인 `for-loop`를 사용할 때보다 오버헤드가 훨씬 심각하게 발생하기 때문에, 모든 `for-loop`를 대신해서 `Stream.forEach()`를 기본적인 반복문 처리 방식으로 사용하면, 애플리케이션 전체에 걸쳐 누적되는 CPU 싸이클 낭비는 무시하지 못할 수준이 될 수 있다. 단순히 반복문 처리 스타일의 선택만으로 CPU 소모량이 10%~20% 정도 더 많아진다면, 그 선택은 근본적으로 잘못된 것이다. 반복문 각각을 놓고 보면 큰 차이가 없지만, 시스템 전체로 보면 피하는 것이 좋다.
...
SW 엔지니어인 우리는 코드 스타일을 아주 중요한 것으로 여기고 [토론](https://blog.jooq.org/2014/07/25/top-10-very-very-very-important-topics-to-discuss/)하기도 한다.

왜냐하면 SW는 유지관리하기가 어렵기 때문이다. 특히나 다른 사람이 작성한 코드는 더욱 그렇다. 그 다른 사람이 Java로 전향하기 전에 오로지 C로만 코딩해온 사람이라면..

물론 지금까지 예를 들었던 코드에서는 가독성 문제라고 할 것은 없었다. 아래의 두 코드는 가독성 면에서 별로 다를 것이 없다:
...
뭐야, 우리가 지금 뭐 한거지? 그렇다. *1. 성능 - 떨어진다* 에서 얘기한 것처럼 modern 방식에 성능 이슈가 존재하는 이유를 보여주고 있다. 내부 반복(Internal iteration)을 사용하면 겉으로는 드러나지 않지만 JVM과 라이브러리가 할일이 많아진다. 위 예제와 같이 상당히 단순한 케이스임에서도 modern 방식을 쓰면 복잡한 호출구조가 존재하는데, 하물며 *2. 가독성 - 적어도 대부분의 사람들에게는..* 에서 다뤘던 AA, AB, ..., ZZ 시퀀스 생성 예제처럼 복잡한 케이스에서는 어떨까..
...
Java 개발자는 많은 연습을 통해 언제 함수형 프로그래밍을 쓰는 것이 좋은지, 어떨때 객체지향/절차형 방식을 고수하는 것이 좋은지 직관적으로 이해할 수 있어야 한다. 충분한 양의 연습이 동반된다면, 두 가지를 혼용하는 것은 우리가 만드는 SW를 개선할 수 있을 것이다.

엉클 밥의 얘기를 빌리자면:

>정작 중요한 것은 이것이다. 뭔지 잘 알고 있다면 객체지향 프로그래밍은 좋다. 뭔지 잘 알고 있다면 함수형 프로그래밍은 좋다. 뭔지 잘 알고 있다면 함수형 객체지향 프로그래밍은 좋다.
