...
위 말을 좀 다르게 표현하면, **'화살표 함수 안에 있는 `this`는 화살표 함수가 정의되어 있는 스코프(scope)에서 어휘적(lexical)으로 이미 정해져 있는 `this` 값으로 정해진다.'**라고 할 수 있다. 화살표 함수 안에 있는 `this`를 **`lexical this`**라고 부르기도 한다.
